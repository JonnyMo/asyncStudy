...
```
看起来代码的变长了， 但发现没有，我们没有一层层的嵌套回调函数了，另外处理错误的代码只用写一次就够了
...
###详说Promise的使用
####一个Promise实例有三种状态 分别是： pendding， resovled, reject

一旦promise实例对象的状态发生改变，就不能再次改变了

pedding ==> resovled  调用resolved(value) 就是状态变为 resolved (成功状态)，值也变成value

或

pedding ==> rejected  调用rejected(reason) 就是状态变为 rejected（失败状态），失败的理由就是reason

###new Promise(executor) 返回一个promise实例
参数 executor: function(resolve, reject){}

resolve 和 reject 是Promise内部实现的，由用户自己决定什么时候调用

executor 这个函数是立即执行的，及代码是同步执行的,下面代码可以验证
...
new 生成的Promise实例对象的状态没有改变，导致 onResolved和 onRejected 回调都不会执行

下面的代码调用了 resolve，把状态改为成功，值改为1
```javascript
new Promise((resolve, reject) => {
	console.log(1);
	resolve(1)
...
```
输出的结果是： 1 2 1
then方法的参数：（onResolved, onRejected） 两个回调函数，这两个回调函数的代码是异步执行的。

如果Promise 的状态是成功，就调用onResolved(value) 参数是成功的值

如果Promise 的状态是失败，就调用onRejected(reason) 参数是出错的理由，可能是Error类型 也可能不是

####then方法返回值也是一个Promise，所以.then()之后还可以继续.then 达到链式调用的要求

onResolved 和 onRejected 回调函数执行的结果，会影响then返回的promise实例的状态和结果

回调函数的返回值分两种情况：

1. 如果回调函数的执行结果是非Promise实例对象，那么then返回的promise实例对象就是成功状态，值是回调函数的返回值

2. 如果是promise实例对象，那么then返回的promise实例对象的状态和值 跟 回调函数返回值的状态和值一样
